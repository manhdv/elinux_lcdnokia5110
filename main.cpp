...
void loop() {
    ...
    // Manage cursor
    if (cursorX >= (LCD_WIDTH - 4)) { // If the next char will be off screen...
...
void lcdFunTime() {
    ...
    for (int i = 0; i < pixelCount; i++) {
        ...
        // white.
        setPixel(rand() % LCD_WIDTH, rand() % LCD_HEIGHT);
    ...
    for (int x = 0; x < LCD_WIDTH; x += 8) { // Swipe right black
        setRect(0, 0, x, LCD_HEIGHT, 1, BLACK);
        updateDisplay();
        delay(10);
    }
    for (int x = 0; x < LCD_WIDTH; x += 8) { // Swipe right white
        setRect(0, 0, x, LCD_HEIGHT, 1, WHITE);
        updateDisplay();
        delay(10);
    }
    for (int x = 0; x < 12; x++) { // Shutter swipe
