...
//const int scePin = BCM_13; // SCE - Chip select, pin 3 on LCD.
//const int rstPin = BCM_11; // RST - Reset, pin 4 on LCD.
//const int dcPin = BCM_15; // DC - Data/Command, pin 5 on LCD.
//const int sdinPin = BCM_19; // DN(MOSI) - Serial data, pin 6 on LCD.
//const int sclkPin = BCM_23; // SCLK - Serial clock, pin 7 on LCD.
//const int blPin = BCM_29; // LED - Backlight LED, pin 8 on LCD.

const int scePin = 2; // SCE - Chip select, pin 3 on LCD.
const int rstPin = 0; // RST - Reset, pin 4 on LCD.
const int dcPin = 3; // DC - Data/Command, pin 5 on LCD.
const int sdinPin = 12; // DN(MOSI) - Serial data, pin 6 on LCD.
const int sclkPin = 14; // SCLK - Serial clock, pin 7 on LCD.
const int blPin = 21; // LED - Backlight LED, pin 8 on LCD.
...
void LCDWrite(uint8_t data_or_command, uint8_t data) {
    ...
    wiringPiSPIDataRW(SPI_CHANEL, &data, 1); //shiftOut(sdinPin, sclkPin, MSBFIRST, data);
...
//    for(;;)
//    {
//        digitalWrite(rstPin, LOW); delay(1000);
//        digitalWrite(rstPin, HIGH); delay(1000);
//    }
    
    fd = wiringPiSPISetupMode(SPI_CHANEL, 500000, 0);
    printf("fd = %d", fd);
